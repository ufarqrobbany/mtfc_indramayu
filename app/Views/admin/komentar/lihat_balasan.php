            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="text-danger"><strong><?= $berita[0]->judul; ?></strong></div>
                        <strong><?= $komentar[0]->nama ?><?= $komentar[0]->foto != NULL ? '<span class="bg-danger text-white rounded px-2 py-1 d-inline-block ml-2" style="font-size: .7rem">admin</span>' : '' ?></strong>
                        <div><?= $komentar[0]->komentar ?></div>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php $x = 1; ?>
                                <?php foreach ($subkomentar as $sub) : ?>
                                    <?php
                                    $db      = \Config\Database::connect();
                                    $update = $db->table('subkomentar_berita')->where('id_subkomentar', $sub->id_subkomentar)->update(['dilihat' => 1]);
                                    ?>
                                    <tr>
                                        <td class="text-center"><?= $x; ?></td>
                                        <td class="text-center"><?= tgl_indo($sub->waktu) ?></td>
                                        <td class="text-center"><?= $sub->nama ?><?= $sub->foto != NULL ? '<span class="bg-danger text-white rounded px-2 py-1 d-inline-block ml-2" style="font-size: .7rem">admin</span>' : '' ?></td>
                                        <td><?= $sub->komentar ?></td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center">
                                                <a href="<?= site_url('/admin/komentar/hapus_ini_sub/') . $sub->id_subkomentar ?>" onclick="isconfirm();" class="btn btn-danger d-block confirmHapus">
                    ...
                    <div class="card-footer bg-white border-top">
                        <div class="d-flex row justify-content-end">
                            <a href="#" class="btn btn-success d-block" data-toggle="modal" data-target="#modal-tambah-komen">
                                <i class="fas fa-plus"></i><span class="ml-2">Balas Komentar</span>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('subkomentar/add') ?>" method="post" class="d-flex flex-column row-gap-3">
                    <?= csrf_field() ?>
